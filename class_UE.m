...
classdef class_UE
    properties
        ...
        DELAY;
        PACKET_LOSS;
        current_serving_time = 0; % 현재 serving site에 머문 시간
        current_serving_start_time = 0; % 현재 site로 바뀐 시점
        ...
        % Mobility : 파라미터를 여기서 설정해줘야함.
        speed_mps = 0;          % [m/s]
        heading_deg = 0;        % [deg] 0=+x, 90=+y (수학좌표계 기준)
        last_dt_moved = 0;      % 누적 이동 시간 (디버그용)
        ...
        % Mobility Interruption Time (MIT) breakdown accumulators
        MIT_HO_EVENTS = 0;
        MIT_T_BREAK_SUM = 0;
        MIT_T_PROC_SUM = 0;
        MIT_T_INTERRUPT_SUM = 0;
        MIT_T_RACH_SUM = 0;
        MIT_T_HC_SUM = 0;
        MIT_TOTAL_SUM = 0;
        RACH_RB_EQ_SUM = 0;
    ...
    methods
        ...
        function obj = class_UE(loc_x, loc_y, altitude)
            ...
            obj.MIT_HO_EVENTS = 0;
            obj.MIT_T_BREAK_SUM = 0;
            obj.MIT_T_PROC_SUM = 0;
            obj.MIT_T_INTERRUPT_SUM = 0;
            obj.MIT_T_RACH_SUM = 0;
            obj.MIT_T_HC_SUM = 0;
            obj.MIT_TOTAL_SUM = 0;
            obj.RACH_RB_EQ_SUM = 0;
        ...
        function obj = update_serv_site_idx(obj, BORE_X, BORE_Y)
            ...
            % if isempty(obj.SERV_SITE_IDX) || obj.SERV_SITE_IDX ~= min_idx
            %     obj.SERV_SITE_IDX = min_idx;
            %     obj.current_serving_start_time = current_time;
            %     obj.current_serving_time = 0;
            % else
            %     % 같은 서빙 셀 유지 중: 체류시간 증가 구현
            %     obj.current_serving_time = current_time - obj.current_serving_start_time;
            % end
        ...
        % 250930 신규추가: UE 모빌리티 추가
        function obj = set_mobility(obj, speed_kmh, heading_deg)
            obj.speed_mps = speed_kmh / 3.6;
            obj.heading_deg = heading_deg;
        end

        function obj = step_move(obj, sample_time)
            if obj.speed_mps <= 0, return; end
            vx = obj.speed_mps * cosd(obj.heading_deg);
            vy = obj.speed_mps * sind(obj.heading_deg);
            obj.LOC_X = obj.LOC_X + vx * sample_time;
            obj.LOC_Y = obj.LOC_Y + vy * sample_time;
            obj.last_dt_moved = obj.last_dt_moved + sample_time;
        end
        ...
        % 
        %                 % 업데이트된 객체를 다시 할당
        %                 % obj = obj.update_serv_site_idx(sat.BORE_X, sat.BORE_Y, current_time);
        %                 obj = obj.update_serv_site_idx(sat.BORE_X, sat.BORE_Y);
        %                 obj.current_serving_start_time = current_time;
        %                 obj.current_serving_time = 0;
        %                 obj.last_site_idx = obj.SERV_SITE_IDX;
        %             end
        %         end
        %     end
        % end
            ...
            elseif ~isempty(obj.rlf_instance)
                if obj.SINR(obj.SERV_SITE_IDX) >= Q_out
                    ...
                    if obj.rlf_instance.T310_check >= T310
                        ...
                        % RLF 복구 전/후 서빙 비교해서 로그
                        old_serv = obj.SERV_SITE_IDX;
        
                        obj.rlf_instance = [];
                        obj = obj.update_serv_site_idx(sat.BORE_X, sat.BORE_Y);
                        obj.handover = obj.handover.reset(obj.SERV_SITE_IDX);
        
                        if obj.SERV_SITE_IDX ~= old_serv
                            fprintf('[RLF-RECOVERY] t=%.3f: serv %d -> %d (NO HO++), reason=T310, RLF=%d\n', ...
                                current_time, old_serv, obj.SERV_SITE_IDX, obj.RLF);
                        else
                            fprintf('[RLF-RECOVERY] t=%.3f: keep serv %d (NO HO++), reason=T310, RLF=%d\n', ...
                                current_time, obj.SERV_SITE_IDX, obj.RLF);
                        end
