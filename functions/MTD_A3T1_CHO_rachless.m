%% A3-T1 based CHO (RACH-less): prep는 A3, execution은 serving ML 기반 타이머 조건
function ue = MTD_A3T1_CHO_rachless(ue, sat, Offset_A3, TTT, current_time, msgCfg)
    C_LIGHT = 299792458;
    PROC_SERV = 0.001;
    PROC_TARG = 0.001;
    PROC_EXEC_UE = 0.001;
    ...
    if nargin >= 6 && ~isempty(msgCfg)
        ...
        if isfield(msgCfg, 'EXEC_SECOND_EVENT_ENABLE') && ~isempty(msgCfg.EXEC_SECOND_EVENT_ENABLE)
            EXEC_SECOND_EVENT_ENABLE = logical(msgCfg.EXEC_SECOND_EVENT_ENABLE);
        end
    ...
    if handover.preparation_state
        ...
        if current_time < exec_ready_time || ~exec_trigger_t1
            return;
        end

        grantMode = lower(strtrim(RACHLESS_GRANT_MODE));
        if strcmp(grantMode, 'dynamic') && ENABLE_DYN_GRANT_QUEUE && ~DYN_GRANT_PREP_SEND
            ue = enqueue_dynamic_grants(ue, current_time, pd_targ_ue, PROC_GRANT_SCHED, DYN_GRANT_PERIOD, ...
                DYN_GRANT_VALIDITY, DYN_GRANT_MAX_TX, RB_PER_GRANT_TX, DYN_GRANT_PREP_SEND, PROC_INTERRUPT, QUEUE_LOG, true);
            ue = prune_dynamic_grant_queue(ue, current_time);
            queue_log_print(QUEUE_LOG, current_time, 'EXEC1-GRANT', ue.handover.dynamic_grant_queue, 'stream started by exec1');
        end

        if EXEC_SECOND_EVENT_ENABLE
            % Execution-time A3 re-evaluation (keep ML trigger unchanged, then re-check A3)
            cond1_exec_vec = ue.RSRP_FILTERED > (ue.RSRP_FILTERED(serv) + Offset_A3);
            cond1_exec_vec(serv) = false;
            exec_valid_indices = find(cond1_exec_vec);

            if isempty(exec_valid_indices)
                ue.handover.prep_cond_fail_count = ue.handover.prep_cond_fail_count + 1;
                if ue.handover.prep_cond_fail_count >= EXEC_A3_FAIL_MAX
                    ue.handover.exec_TTT_check = 0;
                    ue.handover = ue.handover.reset(serv);
                    if VERBOSE
                        fprintf('[CHO-CANCEL][A3T1-RACHLESS] t=%.6f: exec-A3 failed %d consecutive times, reset to serv=%d\n', ...
                            current_time, EXEC_A3_FAIL_MAX, serv);
                    end
                end
                return;
            end
            ...
            if exec_best_targ ~= ue.handover.targ_idx
                if VERBOSE
                    fprintf('[EXEC-A3-RETARGET][A3T1-RACHLESS] t=%.6f: targ %d -> %d\n', ...
                        current_time, ue.handover.targ_idx, exec_best_targ);
                end
                ue.handover.targ_idx = exec_best_targ;
            end
        else
            ue.handover.prep_cond_fail_count = 0;
        end
        ...
        if ue.handover.msg_phase == "CMD_RX_DONE" && ~ue.handover.execution_state
            exec_time_evt = max([current_time, exec_ready_time, ue.handover.cho_cmd_rx_time]);
        ...
        if ue.handover.msg_phase == "WAIT_DYN_GRANT"
            ue = enqueue_dynamic_grants(ue, current_time, pd_targ_ue, PROC_GRANT_SCHED, DYN_GRANT_PERIOD, ...
                DYN_GRANT_VALIDITY, DYN_GRANT_MAX_TX, RB_PER_GRANT_TX, DYN_GRANT_PREP_SEND, PROC_INTERRUPT, QUEUE_LOG, false);
            ue = prune_dynamic_grant_queue(ue, current_time);
...
function ue = enqueue_dynamic_grants(ue, current_time, pd_targ_ue, proc_grant_sched, grant_period, grant_validity, max_tx_per_step, rb_per_grant, prep_send, ~, queueLog, allowExec1Start)
    hand = ue.handover;
    ...
    if isnan(hand.dyn_grant_next_tx_time)
        if prep_send
            ...
            if ~hand.execution_state && ~allowExec1Start
                return;
            end
            if hand.execution_state && hand.exec_time > 0
                stream_start = hand.exec_time + proc_grant_sched;
            else
                stream_start = current_time + proc_grant_sched;
            end
        end
