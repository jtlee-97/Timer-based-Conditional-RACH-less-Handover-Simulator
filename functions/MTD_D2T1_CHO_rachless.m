%% D2-T1 based CHO (RACH-less): prep는 D2, execution은 serving ML 기반 타이머 조건
function ue = MTD_D2T1_CHO_rachless(ue, sat, Hys, Thresh1, Thresh2, current_time, msgCfg)
    C_LIGHT = 299792458;
    PROC_SERV = 0.001;
    PROC_TARG = 0.001;
    PROC_EXEC_UE = 0.001;
    ...
    cond1_enter = ue.ML(serv) - Hys > Thresh1;
    cond2_vec = ue.ML + Hys < Thresh2;
    cond2_vec(serv) = false;

    cond_leave_d2_3 = ue.ML(serv) + Hys < Thresh1;
    cond_leave_d2_4 = false;
    if handover.targ_idx > 0
        cond_leave_d2_4 = ue.ML(handover.targ_idx) - Hys > Thresh2;
    end;

    if ~handover.preparation_state && ~handover.execution_state
        if cond1_enter && any(cond2_vec)
            ue.RBs = ue.RBs + 1;
            ...
            valid_indices = find(cond2_vec);
            [~, min_idx] = min(ue.ML(valid_indices));
            new_site_idx = valid_indices(min_idx);

            if new_site_idx ~= serv && (current_time - ue.handover.TTT_check) >= 0
                ...
                if VERBOSE
                    fprintf('[D2T1-PREP][RACHLESS] t=%.6f: serv=%d, targ=%d, D2 condition met\n', current_time, serv, new_site_idx);
    ...
    if handover.preparation_state
        ...
        if cond_leave_d2_3 || cond_leave_d2_4
            ue.handover.exec_TTT_check = 0;
            ue.handover = ue.handover.reset(serv);
            if VERBOSE
                fprintf('[CHO-CANCEL][D2T1-RACHLESS] t=%.6f: D2 leaving condition met (D2-3=%d, D2-4=%d), reset to serv=%d\n', ...
                    current_time, cond_leave_d2_3, cond_leave_d2_4, serv);
            end
            return;
        end
        ...
        if ue.handover.msg_phase == "CMD_RX_DONE" && ~ue.handover.execution_state
            ...
            if strcmp(grantMode, 'dynamic')
                ...
                ue.handover.dyn_grant_next_tx_time = NaN;
...
function ue = enqueue_dynamic_grants(ue, current_time, pd_targ_ue, proc_grant_sched, grant_period, grant_validity, max_tx_per_step, rb_per_grant, prep_send, ~, queueLog)
    hand = ue.handover;
