%% D2-based CHO (RACH-less): execution 후 CFRA 생략, pre-granted UL로 즉시 UL 연결
function ue = MTD_D2_CHO_rachless(ue, sat, Hys, Thresh1, Thresh2, current_time, msgCfg)
    C_LIGHT = 299792458;
    PROC_SERV = 0.001;
    PROC_TARG = 0.001;
    PROC_EXEC_UE = 0.001;
    ...
    if ~handover.preparation_state && ~handover.execution_state
        if cond1_enter && any(cond2_vec)
            ...
            if new_site_idx ~= serv && (current_time - ue.handover.TTT_check) >= 0
                ...
                if VERBOSE
                    fprintf('[D2-PREP][RACHLESS] t=%.6f: serv=%d, targ=%d, D2 condition met\n', current_time, serv, new_site_idx);
    ...
    if handover.preparation_state
        ...
        if cond_leave_d2_3 || cond_leave_d2_4
            ...
            if ue.handover.prep_cond_fail_count >= PREP_CANCEL_FAIL_MAX
                ...
                ue.handover = ue.handover.reset(serv);
                if VERBOSE
                    fprintf('[CHO-CANCEL][RACHLESS] t=%.6f: D2 leaving condition met %d consecutive times (D2-3=%d, D2-4=%d), reset to serv=%d\n', ...
                        current_time, PREP_CANCEL_FAIL_MAX, cond_leave_d2_3, cond_leave_d2_4, serv);
                end
            end
            return;
        end
