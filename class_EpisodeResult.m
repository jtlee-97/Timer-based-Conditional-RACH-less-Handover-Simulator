classdef class_EpisodeResult
    ...
    methods
        function obj = calculate_average(obj, history, sample_time)
            ...
            if ~isempty(history.MIT_HO_EVENTS)
                obj.MIT_HO_EVENTS = max(history.MIT_HO_EVENTS);
                obj.MIT_T_BREAK_SUM = max(history.MIT_T_BREAK_SUM);
                obj.MIT_T_PROC_SUM = max(history.MIT_T_PROC_SUM);
                obj.MIT_T_INTERRUPT_SUM = max(history.MIT_T_INTERRUPT_SUM);
                obj.MIT_T_RACH_SUM = max(history.MIT_T_RACH_SUM);
                obj.MIT_T_HC_SUM = max(history.MIT_T_HC_SUM);
                if isprop(history, 'MIT_TOTAL_SUM') && ~isempty(history.MIT_TOTAL_SUM)
                    obj.MIT_TOTAL_SUM = max(history.MIT_TOTAL_SUM);
                else
                    obj.MIT_TOTAL_SUM = obj.MIT_T_BREAK_SUM + obj.MIT_T_PROC_SUM + obj.MIT_T_INTERRUPT_SUM + obj.MIT_T_RACH_SUM + obj.MIT_T_HC_SUM;
                end
                if isprop(history, 'RACH_RB_EQ_SUM') && ~isempty(history.RACH_RB_EQ_SUM)
                    obj.RACH_RB_EQ_SUM = max(history.RACH_RB_EQ_SUM);
                end
            end

            if ~isempty(history.DYN_GRANT_TX_COUNT)
                obj.DYN_GRANT_TX_COUNT = max(history.DYN_GRANT_TX_COUNT);
                obj.DYN_GRANT_FAIL_COUNT = max(history.DYN_GRANT_FAIL_COUNT);
                obj.DYN_GRANT_FALLBACK_COUNT = max(history.DYN_GRANT_FALLBACK_COUNT);
            end
        end
