...
%% ===== Figure typography/layout options =====
MAIN_FONT_SIZE      = 15;
LEGEND_FONT_SIZE    = 15;
ANNOT_FONT_SIZE     = 15;
VALUE_FONT_SIZE     = 15;
...
% dual y-axis ylabel position
LABEL_PAD_LEFT      = -0.08;
LABEL_PAD_RIGHT     = 1.08;

% subfigure label placement
SUBFIG_Y_OFFSET     = 0.018;   % 기존 0.035 -> 너무 아래였음
SUBFIG_BOX_W        = 0.05;
SUBFIG_BOX_H        = 0.03;
...
idxCmp00 = [1 2 3];
idxCmp10 = [4 5 6];
labelsCmp = {'CHO-CFRA', 'CHO-RAL', '\bf Proposed'};
...
set(fig1, 'Units', 'pixels', 'Position', [130 130 1120 810]);

% ax1 = axes(fig1, 'Position', [0.10 0.22 0.34 0.67]);
ax1 = axes(fig1, 'Position', [0.10 0.22 0.34 0.67]);
[h1a, h1b] = plot_dualbar_metric(ax1, idxCmp00, labelsCmp, mMIT, mRBsPerHO, ...
    'Average MIT [ms]', 'Average RBs/HO', ...
    MAIN_FONT_SIZE, VALUE_FONT_SIZE, LABEL_PAD_LEFT, LABEL_PAD_RIGHT);

% ax2 = axes(fig1, 'Position', [0.58 0.22 0.34 0.67]);
ax2 = axes(fig1, 'Position', [0.58 0.22 0.34 0.67]);
...
set(lgd1, ...
    ...
    'Box', 'off', ...
    'Units', 'normalized', ...
    'Position', [0.39 0.91 0.22 0.04], ...
    'FontSize', LEGEND_FONT_SIZE);

% add_subfigure_label(fig1, ax1, '(a)', ANNOT_FONT_SIZE, 0.014, SUBFIG_BOX_W, SUBFIG_BOX_H);
% add_subfigure_label(fig1, ax2, '(b)', ANNOT_FONT_SIZE, 0.014, SUBFIG_BOX_W, SUBFIG_BOX_H);
...
set(fig3, 'Units', 'pixels', 'Position', [130 130 1120 810]);

ax3 = axes(fig3, 'Position', [0.09 0.22 0.36 0.67]);
ax4 = axes(fig3, 'Position', [0.56 0.22 0.36 0.67]);
...
if ~isempty(validAllFig3) && max(validAllFig3) > 0
    yMaxGlobalFig3 = max(validAllFig3) * 1.28;
else
    yMaxGlobalFig3 = 1;
end
...
[h31, h32, h34] = plot_hoeff_breakdown(ax3, idxCmp00, labelsCmp, ...
    ...
    MAIN_FONT_SIZE, VALUE_FONT_SIZE);
...
set(lgd3, ...
    ...
    'Box', 'off', ...
    'Units', 'normalized', ...
    'Position', [0.20 0.92 0.60 0.04], ...
    'FontSize', LEGEND_FONT_SIZE);

% add_subfigure_label(fig3, ax3, '(a)', ANNOT_FONT_SIZE, 0.014, SUBFIG_BOX_W, SUBFIG_BOX_H);
% add_subfigure_label(fig3, ax4, '(b)', ANNOT_FONT_SIZE, 0.014, SUBFIG_BOX_W, SUBFIG_BOX_H);
...
set(fig5, 'Units', 'pixels', 'Position', [130 130 820 610]);

ax5 = axes(fig5, 'Position', [0.12 0.18 0.76 0.72]);
x = 1:numel(idxA3T1_5);

yyaxis(ax5, 'left');
pWasteVar = plot(ax5, x, mWastedProb(idxA3T1_5), '-o', ...
    'LineWidth', 2.2, ...
    'MarkerSize', 8.0, ...
    'Color', [0.85 0.33 0.10], ...
    'MarkerFaceColor', [0.85 0.33 0.10]);
ylabel(ax5, 'Wasted HO ratio [%]', 'FontSize', MAIN_FONT_SIZE);
ax5.YAxis(1).Color = [0.85 0.33 0.10];

validWasteVar = mWastedProb(idxA3T1_5);
validWasteVar = validWasteVar(isfinite(validWasteVar));
yyaxis(ax5, 'left');
safe_set_ylim_nonneg(ax5, validWasteVar, 1.38);

yoffW = safe_text_offset(validWasteVar, 0.045, 0.03);
for i = 1:numel(idxA3T1_5)
    yv = mWastedProb(idxA3T1_5(i));
    if isfinite(yv)
        text(ax5, i, yv + yoffW, sprintf('%.2f', yv), ...
...
yyaxis(ax5, 'right');
pSINR = plot(ax5, x, avgSINR(idxA3T1_5), '-s', ...
    'LineWidth', 2.2, ...
    'MarkerSize', 7.5, ...
    'Color', [0.12 0.38 0.80], ...
    'MarkerFaceColor', [0.12 0.38 0.80]);
ylabel(ax5, 'Average DL SINR [dB]', 'FontSize', MAIN_FONT_SIZE);
ax5.YAxis(2).Color = [0.12 0.38 0.80];

validSINR = avgSINR(idxA3T1_5);
validSINR = validSINR(isfinite(validSINR));
yyaxis(ax5, 'right');
safe_set_ylim_signed(ax5, validSINR, 0.28);

yoffS = safe_text_offset_range(validSINR, 0.045, 0.015);
for i = 1:numel(idxA3T1_5)
    yv = avgSINR(idxA3T1_5(i));
    if isfinite(yv)
        text(ax5, i, yv + yoffS, sprintf('%.2f', yv), ...
            'HorizontalAlignment', 'center', ...
            'VerticalAlignment', 'bottom', ...
            'FontSize', VALUE_FONT_SIZE, ...
            'FontWeight', 'bold', ...
...
set(ax5, ...
    'XTick', x, ...
    'XTickLabel', labelsA3T1, ...
    'XTickLabelRotation', 10, ...
    'FontSize', MAIN_FONT_SIZE, ...
    'LineWidth', AXES_LINE_WIDTH, ...
    'TickLabelInterpreter', 'tex');
xlim(ax5, [0.55, numel(idxA3T1_5)+0.45]);

ax5.YAxis(1).Label.Units = 'normalized';
ax5.YAxis(2).Label.Units = 'normalized';
ax5.YAxis(1).Label.Position(1) = LABEL_PAD_LEFT;
ax5.YAxis(2).Label.Position(1) = LABEL_PAD_RIGHT;

legend(ax5, [pWasteVar, pSINR], ...
...
function [b1, b2] = plot_dualbar_metric(ax, idxSet, xLabels, leftDataAll, rightDataAll, ...
    leftYLabel, rightYLabel, mainFontSize, valueFontSize, labelPadLeft, labelPadRight)
    ...
    b1 = bar(ax, x - 0.17, leftDataAll(idxSet), 0.34, ...
        ...
        'FaceAlpha', 0.82);
    ylabel(ax, leftYLabel, 'FontSize', mainFontSize);
    ax.YAxis(1).Color = [0.55 0.18 0.18];

    validLeft = leftDataAll(idxSet);
    validLeft = validLeft(isfinite(validLeft));
    yyaxis(ax, 'left');
    safe_set_ylim_nonneg(ax, validLeft, 1.32);

    yoffLeft = safe_text_offset(validLeft, 0.06, 0.06);
    ...
    b2 = bar(ax, x + 0.17, rightDataAll(idxSet), 0.34, ...
        ...
        'FaceAlpha', 0.78);
    ylabel(ax, rightYLabel, 'FontSize', mainFontSize);
    ax.YAxis(2).Color = [0.10 0.25 0.55];

    validRight = rightDataAll(idxSet);
    validRight = validRight(isfinite(validRight));
    yyaxis(ax, 'right');
    safe_set_ylim_nonneg(ax, validRight, 1.36);

    yoffRight = safe_text_offset(validRight, 0.08, 0.08);
    for ii = 1:numel(idxSet)
    ...
    set(ax, ...
        ...
        'LineWidth', 1.0, ...
        'TickLabelInterpreter', 'tex');
    xlim(ax, [0.45, numel(idxSet)+0.55]);

    ax.YAxis(1).Label.Units = 'normalized';
    ax.YAxis(2).Label.Units = 'normalized';
    ax.YAxis(1).Label.Position(1) = labelPadLeft;
    ax.YAxis(2).Label.Position(1) = labelPadRight;
end
...
function add_subfigure_label(figH, axH, labelStr, fontSize, yOffset, boxW, boxH)
    drawnow;
    axPos = get(axH, 'Position');
    xCenter = axPos(1) + axPos(3)/2;
    yPos = max(0.005, axPos(2) - yOffset);

    annotation(figH, 'textbox', [xCenter - boxW/2, yPos, boxW, boxH], ...
        'String', labelStr, ...
        'EdgeColor', 'none', ...
        'HorizontalAlignment', 'center', ...
