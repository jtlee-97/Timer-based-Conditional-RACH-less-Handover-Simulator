...
for k = 1:nFile
    ...
    if ~isempty(sinrDataAll{k})
        avgSINR(k) = mean(sinrDataAll{k});
    else
        avgSINR(k) = NaN;
    end
...
% -------------------------------------------------------------------------
% HO Efficiency Breakdown (blue text = wasted ratio)
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
% Additional view: HO Efficiency Breakdown + Average DL SINR
% -------------------------------------------------------------------------
figure('Name', 'HO Efficiency + Avg DL SINR', 'Color', 'w');

x = 1:nFile;

yyaxis left;
stackedHO2 = [mEssentialHORate(:), mUHOPureRate(:), mHOPPRate(:)];
bEff2 = bar(x, stackedHO2, 'stacked', 'BarWidth', 0.62);
bEff2(1).FaceColor = [0.2 0.6 0.2];
bEff2(2).FaceColor = [0.8 0.2 0.2];
bEff2(3).FaceColor = [0.5 0.0 0.5];

switch lower(strtrim(HO_RATE_UNIT))
    case 'per_min'
        effLabel2 = 'Average HOs [#/UE/min.]';
    case 'per_sec'
        effLabel2 = 'Average HOs [#/UE/sec.]';
end
ylabel(effLabel2);

validTot2 = mTotalHORateBreakdown(~isnan(mTotalHORateBreakdown));
if ~isempty(validTot2) && max(validTot2) > 0
    ylim([0, max(validTot2) * 1.25]);
else
    ylim([0, 1]);
end
...
for i = 1:nFile
    ...
    if ~isempty(validTot2)
        yOffset = max(validTot2) * 0.015;
    else
        yOffset = 0.02;
    end

    text(i, totalVal + yOffset, sprintf('%.1f%%', wastePct), ...
        'HorizontalAlignment', 'center', ...
        'VerticalAlignment', 'bottom', ...
        'FontSize', 10, ...
        'FontWeight', 'bold', ...
...
sinrYOffset = 0;
if ~isempty(validSINR)
    sinrRange = max(validSINR) - min(validSINR);
    if sinrRange < 1e-9
        sinrYOffset = 0.2;
    else
        sinrYOffset = 0.03 * sinrRange;
    end
else
    sinrYOffset = 0.1;
end
...
for i = 1:nFile
    if ~isnan(avgSINR(i))
        text(i, avgSINR(i) + sinrYOffset, sprintf('%.2f', avgSINR(i)), ...
            'HorizontalAlignment', 'center', ...
            'VerticalAlignment', 'bottom', ...
            'FontSize', 9, ...
            'FontWeight', 'bold', ...
...
title('HO Efficiency Breakdown with Average DL SINR');

legend([bEff2(1), bEff2(2), bEff2(3), bSINR], ...
    {'Essential HO (Valid)', 'Wasted HO (UHO)', 'Wasted HO (HOPP)', 'Average DL SINR'}, ...
    'Location', 'northwest');

savefig(fullfile(resultsDir, ['compare_HO_efficiency_with_avgSINR_' hoRateFileSuffix '.fig']));
saveas(gcf, fullfile(resultsDir, ['compare_HO_efficiency_with_avgSINR_' hoRateFileSuffix '.png']));
...
% -------------------------------------------------------------------------
% Improved view: MIT + RBs/HO (both as bars)
% 기존 그래프는 유지하고, improved 버전만 bar+bar 로 생성
% -------------------------------------------------------------------------
...
mitYOffset = 0.02 * max([validMIT(:); 1]);
for i = 1:nFile
    if ~isnan(mMIT(i))
        text(x(i) - 0.18, mMIT(i) + mitYOffset, sprintf('%.2f', mMIT(i)), ...
            'HorizontalAlignment', 'center', ...
            'VerticalAlignment', 'bottom', ...
            'FontSize', 9, ...
...
% 오른쪽 축: RBs/HO
yyaxis right;
bRB = bar(x + 0.18, mRBsPerHO, 0.34, ...
    'FaceColor', [0.20 0.45 0.85], ...
    'EdgeColor', [0.10 0.25 0.55], ...
    'FaceAlpha', 0.75);
ylabel('RBs per HO');
axImp.YAxis(2).Color = [0.10 0.25 0.55];
...
rbYOffset = 0.03 * max([validRB(:); 1]);
for i = 1:nFile
    if ~isnan(mRBsPerHO(i))
        text(x(i) + 0.18, mRBsPerHO(i) + rbYOffset, sprintf('%.2f', mRBsPerHO(i)), ...
            'HorizontalAlignment', 'center', ...
            'VerticalAlignment', 'bottom', ...
            'FontSize', 9, ...
...
validRB2 = mRBsPerHO(~isnan(mRBsPerHO));
if isempty(validRB2)
    ylim([0, 1]);
else
    ylim([0, max(validRB2) * 1.15]);
end
